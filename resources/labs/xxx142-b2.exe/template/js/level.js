function Level(levelObject) {
    ...
    for (let i = 0; i < currentLevel.walls.length; i++) {
      for (let j = 1; j < currentLevel.walls[i].length; j++) {
        const colPos = doesLineInterceptCircle(
          currentLevel.walls[i][j - 1],
          currentLevel.walls[i][j],
          position,
          radius
        );
        if (colPos) {
  ...
  const handleSwitches = (oldPos, newPos, radius) => {
    if (oldPos.x !== newPos.x || oldPos.y !== newPos.y) {
      for (const s of currentLevel.switches) {
        ...
        const wasTouching =
          oldPos.sub(switchPos).len() < radius + settings_switchRadius;
        const nowTouching =
          newPos.sub(switchPos).len() < radius + settings_switchRadius;
  ...
  const handleEnd = (position, radius) => {
    const isEnded =
      position.sub(new Vec2(levelObject.end.x, levelObject.end.y)).len() <
      radius + settings_switchRadius;
    if (isEnded) {
      this.completed = true;
    }
  };
  ...
  this.ghostRemoved = (position, radius) => {
    for (const s of currentLevel.switches) {
      const isTouching =
        position.sub(new Vec2(s.x, s.y)).len() < radius + settings_switchRadius;
      if (isTouching) {
        if (s.type !== "single") {
          s.pressed--;
        }
