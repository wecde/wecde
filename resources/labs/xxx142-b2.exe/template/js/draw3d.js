const Drawing = function (canvas) {
  ...
  const uPlayerLightPosition = gl.getUniformLocation(
    shaderProgram,
    "playerLightPosition"
  );
  ...
  const uSurfaceSensitivity = gl.getUniformLocation(
    shaderProgram,
    "inSurfaceSensitivity"
  );
  ...
  const interpolate = (position, movementVector) => {
    return position.sub(
      movementVector.mul(1 - settings_tps * this.accumulator)
    );
  };
  ...
  this.player = (player) => {
    ...
    gl.drawElements(
      gl.TRIANGLES,
      builtSprites.player.ibCount,
      gl.UNSIGNED_SHORT,
      builtSprites.player.ibStart * 2
    );
  };
  ...
  this.ghost = (ghost) => {
    ...
    gl.drawElements(
      gl.TRIANGLES,
      builtSprites.ghost.ibCount,
      gl.UNSIGNED_SHORT,
      builtSprites.ghost.ibStart * 2
    );
    return true;
  };
  ...
  this.level = (level, frameTime, currentTimeDelta, currentCameState) => {
    ...
    fadeLevel = clamp01(
      fadeLevel + (gameState === STATE_FADEOUT ? -timeDelta : timeDelta)
    );
    ...
    gl.uniform3f(
      uTranslation,
      -level.start.x * glScale,
      glScale,
      level.start.y * glScale
    );

    startLight =
      fadeLevel *
      clamp01(startLight + timeDelta * (gameState === STATE_FADEIN ? -2 : 3));
    gl.uniform1f(uSurfaceSensitivity, startLight / 3);
    gl.uniform3f(uAmbientColor, 0.2 * startLight, (1 - startLight) / 4, 0.5);
    gl.drawElements(
      gl.TRIANGLES,
      builtSprites.pad.ibCount,
      gl.UNSIGNED_SHORT,
      builtSprites.pad.ibStart * 2
    );
    ...
    endLight = lerp(
      endLight,
      lerp(0.7, 1, 1 - abs(cos(frameTime * 1.5))),
      timeDelta * 4
    );
    ...
    for (const s of level.switches) {
      ...
      switchState.r = lerp(
        r,
        pressed ? 0.1 : lerp(0.7, 1, 1 - abs(cos(frameTime * 3))),
        timeDelta * 4
      );
      switchState.g = lerp(g, pressed ? 0.3 : 0, timeDelta * 5);
      switchState.p = lerp(
        switchState.p,
        pressed ? 3.8 * glScale : 0,
        timeDelta * 8
      );
