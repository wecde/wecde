        ...
        for (let i = 0; i < this.weapon.count; i++) {
          ...
          if (
            ...
            if ($.powerupTimers[2] > 0) {
              colors.push(
                `hsl(${$.definitions.powerups[2].hue}, ${$.definitions.powerups[2].saturation}%, ${$.definitions.powerups[2].lightness}%)`
              );
            }
            if ($.powerupTimers[3] > 0) {
              colors.push(
                `hsl(${$.definitions.powerups[3].hue}, ${$.definitions.powerups[3].saturation}%, ${$.definitions.powerups[3].lightness}%)`
              );
            }
            if ($.powerupTimers[4] > 0) {
              colors.push(
                `hsl(${$.definitions.powerups[4].hue}, ${$.definitions.powerups[4].saturation}%, ${$.definitions.powerups[4].lightness}%)`
              );
            }
...
$.Hero.prototype.render = function () {
  if (this.life > 0) {
    if (this.takingDamage) {
      ...
      $.ctxmg.fillStyle = `hsla(0, 0%, ${$.util.rand(0, 100)}%, ${$.util.rand(
        0.01,
        0.15
      )})`;
      ...
      var fillStyle = `hsla(${$.util.rand(0, 359)}, 100%, ${$.util.rand(
        20,
        80
      )}%, 1)`;
    } else {
