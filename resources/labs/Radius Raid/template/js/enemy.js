...
$.Enemy.prototype.renderHealth = function (i) {
  if (this.inView && this.life > 0 && this.life < this.lifeMax) {
    ...
    $.ctxmg.fillStyle = `hsla(${
      (this.life / this.lifeMax) * 120
    }, 100%, 50%, 0.75)`;
    $.ctxmg.fillRect(
      this.x - this.radius,
      this.y - this.radius - 6,
  ...
  if (this.inView) {
    ...
    if (this.hitFlag > 0) {
      ...
      $.util.fillCircle(
        ...
        this.y,
        this.radius,
        `hsla(${this.hue}, ${this.saturation}%, 75%, ${this.hitFlag / 10})`
      );
      $.util.strokeCircle(
        $.ctxmg,
        this.x,
        this.y,
        this.radius,
        `hsla(${this.hue}, ${this.saturation}%, ${$.util.rand(60, 90)}%, ${
          this.hitFlag / 10
        })`,
        $.util.rand(1, 10)
      );
