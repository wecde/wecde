...
export default defineComponent({
  ...
  setup() {
    const x = computed({
      get() {
        return $store.state.editor.floatingBrowserX;
      },
      set(value) {
    ...
    const y = computed({
      get() {
        return $store.state.editor.floatingBrowserY;
      },
      set(value) {
    ...
    if (x.value == null || y.value == null) {
      ...
      onMounted(() => {
        ...
        y.value =
          innerHeight / 2 - parseFloat(getComputedStyle(btn.value.$el).height);
      });
    }
    ...
    function onMouseOut() {
      ...
      const styles = getComputedStyle(btn.value.$el);
