...
export function useGitShared() {
  ...
  const { configs } = useGitHelper();

  async function reset(filepaths: readonly string[]): Promise<void> {
  ...
  async function resetIndex(filepaths: readonly string[]): Promise<void> {
    if (store.state.editor.project) {
      await Promise.all(
  ...
  async function add(filepaths: readonly string[]): Promise<void> {
    if (store.state.editor.project) {
      await Promise.all(
        Object.entries(store.state.editor.git.statusMatrix.matrix)
          .filter(([_filepath]) => filepaths.includes(_filepath))
  ...
  async function listAllBranches(): Promise<{
